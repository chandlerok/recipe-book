...
impl RecipeDb {
    ...
    pub async fn save_recipe(&self, recipe: &crate::scraper::ScrapedRecipe) -> Result<()> {
    ...
    pub async fn save_recipe_and_mark_done(
        &self,
        recipe: &crate::scraper::ScrapedRecipe,
        job_id: i32,
    ) -> Result<()> {
        let ingredients = serde_json::to_string(&recipe.ingredients)?;
        let instructions = serde_json::to_string(&recipe.instructions)?;
    ...
    pub async fn get_recipe(&self, url: &str) -> Result<Option<crate::server::RecipeResponse>> {
        let row = sqlx::query(
            r#"
    ...
    pub async fn search(&self, query: &str, limit: i32) -> Result<Vec<crate::server::SearchHit>> {
        let rows = sqlx::query(
            r#"
            SELECT url, title, total_time, ingredients, instructions, image,
        ...
        let hits = rows
            ...
            .map(|r| {
                ...
                crate::server::SearchHit {
                    recipe: crate::server::RecipeResponse {
                        url: r.get("url"),
                        title: r.get("title"),
                        total_time: r.get("total_time"),
                        ingredients,
    ...
    pub async fn queue_stats(&self) -> Result<std::collections::HashMap<String, i64>> {
        ...
        stats.insert("pending".to_string(), 0i64);
        stats.insert("in_progress".to_string(), 0i64);
        stats.insert("done".to_string(), 0i64);
        stats.insert("error".to_string(), 0i64);
...
mod tests {
    ...
    async fn setup_db() -> RecipeDb {
        // Called under the mutex, so this flag check is safe from races
    ...
    fn sample_recipe(url: &str) -> crate::scraper::ScrapedRecipe {
        crate::scraper::ScrapedRecipe {
            url: url.to_string(),
    ...
    async fn test_save_overwrites_existing() {
        let _lock = db_mutex().lock().await;
        let db = setup_db().await;
        let old = crate::scraper::ScrapedRecipe {
            title: "Old".to_string(),
            ..sample_recipe("https://example.com/test")
        };
        let new = crate::scraper::ScrapedRecipe {
            title: "New".to_string(),
            ..sample_recipe("https://example.com/test")
        };
    ...
    async fn test_search_finds_matching_recipe() {
        let _lock = db_mutex().lock().await;
        let db = setup_db().await;
        let recipe = crate::scraper::ScrapedRecipe {
    ...
    async fn test_fts_incremental_updates_after_save() {
        let _lock = db_mutex().lock().await;
        let db = setup_db().await;
        let alpha = crate::scraper::ScrapedRecipe {
        ...
        let beta = crate::scraper::ScrapedRecipe {
    ...
    async fn test_counts_by_status() {
        ...
        assert_eq!(stats.get("pending").copied().unwrap_or(0), 2);
        assert_eq!(stats.get("in_progress").copied().unwrap_or(0), 0);
        assert_eq!(stats.get("error").copied().unwrap_or(0), 1);
        assert_eq!(stats.get("done").copied().unwrap_or(0), 0);
    }
}
